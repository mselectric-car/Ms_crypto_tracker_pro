...
class _HomePageState extends State<HomePage> {
  ...
  Widget _settingsPage() => ListView(
        ...
        children: [
          ...
          const Card(
            child: ListTile(
              title: Text('MS Crypto Tracker Pro v3.2'),
...
class CoinSearchDelegate extends SearchDelegate<SearchCoin?> {
  ...
  Widget _buildSearchBody() {
    ...
    if (cleanQuery.length < 2) {
      return const Center(
        child: Text('Upiši najmanje 2 znaka.'),
      );
    }

    return FutureBuilder<List<SearchCoin>>(
      key: ValueKey(cleanQuery),
      future: Api.search(cleanQuery),
      builder: (context, snapshot) {
        ...
        if (results.isEmpty) {
          return const Center(
            child: Text('Nema pronađenih valuta.'),
          );
        }
